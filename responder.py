...
class Responder:
    ...
    def predict_tensor(self, src, device=torch.device('cpu'), max_output_len = 64, noise_gain=0.0):
        self.s2s.eval()
        self.s2s.to(device)
        src = src.to(device)
    ...
    def predict_sentences(self, sentences, device=torch.device('cpu'), max_output_len = 64, noise_gain=0.0):
        src = [self.spm.EncodeAsIds(sentence) for sentence in sentences]
        src = torch.tensor(src).long()
        tgt = self.predict_tensor(src, device, max_output_len, noise_gain=noise_gain)
